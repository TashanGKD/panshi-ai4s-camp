...
type State = { status: 'loading' } | { status: 'unauthenticated' } | { status: 'forbidden' } | { status: 'authenticated', profile: ProfileResponse }

export const App = ({ client = adminClient }: { client?: AdminClient }) => {
  const [state, setState] = useState<State>({ status: 'loading' })
  const classify = (error: unknown) => setState(error instanceof AdminApiError && error.status === 401
    ? { status: 'unauthenticated' } : { status: 'forbidden' })

  useEffect(() => {
    let active = true
    void client.getProfile().then((profile) => {
      if (active) setState(profile.data.user.role === 'admin' ? { status: 'authenticated', profile } : { status: 'forbidden' })
    }, (error) => { if (active) classify(error) })
    return () => { active = false }
  }, [client])

  if (state.status === 'loading') return <main className="state-shell"><p role="status">正在验证管理员身份</p></main>
  if (state.status === 'forbidden') return <main className="state-shell"><p role="alert">无权访问管理后台</p></main>
  if (state.status === 'unauthenticated') return <LoginPage onLogin={async (input) => {
    await client.login(input)
    const profile = await client.getProfile()
    setState(profile.data.user.role === 'admin' ? { status: 'authenticated', profile } : { status: 'forbidden' })
  }} />
