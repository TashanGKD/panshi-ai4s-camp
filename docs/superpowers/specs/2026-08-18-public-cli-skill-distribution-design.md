...
2. 默认只使用 Skill 内嵌可信 manifest 中固定的 GitHub HTTPS Release 地址、版本、tarball SHA-256 和 `packageTreeSha256`；不得从环境变量、网页内容、远程 manifest、本地安装 marker 或 Agent 提示中接受任意信任值。`packageTreeSha256` 只覆盖 tarball 中的 `panshi-camp-cli` 包目录，不覆盖平台相关公共依赖，由 Task 4 发行门禁计算。
3. 校验和不匹配、下载中断、npm 安装失败时删除临时目录，不替换现有版本。
4. 同版本复用时必须直接重算真实 CLI 包目录摘要并与可信 manifest 比较；本地 marker 不是信任根，包树与 marker 同时被改写仍拒绝。升级必须再次展示预览并确认，安装新版本后原子切换稳定入口，入口切换失败则保持旧入口；降级拒绝。已有非本安装器管理的稳定入口或同名异址 profile 均拒绝覆盖。
...
8. 配置 swap 后、成功返回前执行最终状态复检；确认成功后的备份删除只做 best-effort cleanup，不得因清理失败触发 rollback。后续失败只清理本次新建且仍为空的安装根、bin 与配置目录，不修改已有目录权限。
9. 安装器防御检查期间的 swap 并在末尾复检；不声称阻止同一用户在安装器返回后主动改写自己的文件。
