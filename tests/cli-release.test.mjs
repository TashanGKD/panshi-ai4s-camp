...
test('release source text is checked out with LF on every platform', async () => {
...
test('default release build is local-only and does not modify the tracked Skill manifest', async () => {
  ...
  assert.equal(result.manifest.version, '0.1.14')
  assert.equal(result.manifest.assetName, 'panshi-camp-cli-0.1.14.tgz')
  ...
  // remains responsible for matching the checked-in archive digest exactly.
  ...
  await assert.rejects(checkCliRelease({ repoRoot, skillManifestPath: result.manifestPath, expectedTag: 'cli-v0.1.15' }), /CLI_RELEASE_TAG_DRIFT/u)
})
