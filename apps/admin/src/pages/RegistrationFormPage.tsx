...
import {
  ...
  REGISTRATION_PROFICIENCY_LEVELS,
...
const newAttachment = (order: number): RegistrationAttachment => ({
  id: editorUuid(), label: '新附件', helpText: '', required: false, order, active: true, allowedExtensions: ['pdf', 'docx', 'jpg'], maxSizeBytes: 10 * 1024 * 1024,
})
...
function PreviewQuestion({ question }: { question: RegistrationDynamicQuestion }) {
  ...
  const limits = question.type === 'short_text' || question.type === 'long_text' ? [
  ...
  if (question.type === 'proficiency_matrix') return <fieldset className="registration-question" disabled><legend>{label}</legend>{question.items.map((item) => <p key={item.id}><strong>{item.label}：</strong>{question.levels.map((level) => level.label).join(' / ')}</p>)}{question.allowOther ? <p>支持填写其他能力</p> : null}{help}</fieldset>
...
export function RegistrationFormPage({ client }: { client: AdminClient }) {
  ...
  const changeQuestionType = (id: string, type: RegistrationDynamicQuestion['type']) => updateForm((current) => ({
    ...
    questions: current.questions.map((question): RegistrationDynamicQuestion => {
      ...
      const common = {
        ...
        order: question.order, active: question.active, visibleWhen: question.visibleWhen,
      }
      ...
      if (type === 'proficiency_matrix') return {
        ...common, type, validation: {}, allowOther: true,
        items: [{ id: editorUuid(), value: 'skill-1', label: '能力项目 1' }],
        levels: REGISTRATION_PROFICIENCY_LEVELS.map((level) => ({ id: editorUuid(), ...level })),
      }
      return { ...common, type, validation: question.type === 'short_text' || question.type === 'long_text' ? question.validation : {} }
    }),
  }))
  ...
  const toggleAttachmentExtension = (id: string, extension: 'pdf' | 'docx' | 'jpg') => updateForm((current) => ({
    ...current,
    ...
    <section className="panel registration-editor" aria-labelledby="questions-title"><div className="section-heading"><h2 id="questions-title">动态问题</h2><button type="button" disabled={pending} onClick={() => updateForm((current) => ({ ...current, questions: [...current.questions, newQuestion(current.questions.length)] }))}>新增问题</button></div>
      {form.questions.length === 0 ? <p>尚未配置动态问题。</p> : form.questions.map((question, index) => <fieldset key={question.id} className="registration-editor-item"><legend>问题 {index + 1}</legend>
        <div className="form-grid"><label className="form-field">题目<input aria-label={`问题 ${index + 1} 题目`} id={`question-${question.id}-label`} value={question.label} onChange={(event) => updateQuestion(question.id, { label: event.target.value })} /><ErrorMessages errors={fieldErrors(errors, `questions.${index}.label`)} /></label>
          <label className="form-field">类型<select aria-label={`问题 ${index + 1} 类型`} value={question.type} onChange={(event) => changeQuestionType(question.id, event.target.value as RegistrationDynamicQuestion['type'])}><option value="short_text">单行文本</option><option value="long_text">多行文本</option><option value="single_choice">单选</option><option value="multiple_choice">多选</option><option value="proficiency_matrix">能力矩阵</option></select><ErrorMessages errors={fieldErrors(errors, `questions.${index}.type`)} /></label></div>
        ...
        {question.type === 'multiple_choice' ? <div className="form-grid"><label className="form-field">最少选择<input aria-label={`问题 ${index + 1} 最少选择`} type="number" min={0} value={question.validation.minSelections ?? ''} onChange={(event) => updateQuestion(question.id, { validation: { ...question.validation, minSelections: optionalInteger(event.target.value) } })} /></label><label className="form-field">最多选择<input aria-label={`问题 ${index + 1} 最多选择`} type="number" min={1} value={question.validation.maxSelections ?? ''} onChange={(event) => updateQuestion(question.id, { validation: { ...question.validation, maxSelections: optionalInteger(event.target.value) } })} /></label></div> : null}
