...
export function ApplicationReviewPage({ client }: { client: AdminClient }) {
  ...
  return <section>
    ...
    <section><h2>固定资料</h2>{Object.entries((latest?.profile ?? app.coreFields) as Record<string, unknown>).map(([key, value]) => <p key={key}><strong>{key}：</strong>{String(value)}</p>)}</section>
    <section><h2>报名答案（只读）</h2>{Object.entries((latest?.answers ?? app.answers) as Record<string, unknown>).map(([key, value]) => <p key={key}><strong>{app.form.questions.find((question) => question.id === key)?.label ?? key}：</strong>{Array.isArray(value) ? value.join('、') : String(value)}</p>)}</section>
    ...
    {allowed.length ? <section>
      ...
      {target === 'needs_supplement' ? <div>
        ...
        <fieldset disabled={pending}><legend>允许修改的资料</legend>{['name', 'email', 'organization', 'department', 'identityType', 'educationStage', 'majorResearchDirection', ...app.form.questions.filter((question) => question.active).map((question) => question.id)].map((key) => <label key={key}><input type="checkbox" checked={fields.has(key)} onChange={() => toggle(fields, key, setFields)} />{app.form.questions.find((question) => question.id === key)?.label ?? key}</label>)}</fieldset>
