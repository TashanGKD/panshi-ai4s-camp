...
test('default release build is local-only and does not modify the tracked Skill manifest', async () => {
  ...
  assert.equal(result.manifest.version, '0.1.13')
  assert.equal(result.manifest.assetName, 'panshi-camp-cli-0.1.13.tgz')
  ...
  await assert.rejects(checkCliRelease({ repoRoot, skillManifestPath: result.manifestPath, expectedTag: 'cli-v0.1.14' }), /CLI_RELEASE_TAG_DRIFT/u)
})
