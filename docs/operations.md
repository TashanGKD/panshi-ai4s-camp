...
Registration and review notifications use a separate fail-closed switch, `SMS_NOTIFICATION_PROVIDER`. The checked-in and local default is `disabled`. To enable the transactional outbox worker, set it to `aliyun` and provide `ALIYUN_NOTIFICATION_SMS_SIGN_NAME` plus all five explicit `ALIYUN_NOTIFICATION_SMS_TEMPLATE_*` values. The worker never falls back to the verification template or a default notification template. `Code=OK` is recorded as gateway acceptance with `BizId`; it is not reported as final handset delivery. Explicit provider rate limits may be retried, while ambiguous transport outcomes are moved to dead letter to avoid duplicate non-idempotent sends. Do not backfill historical application states when enabling the worker.
